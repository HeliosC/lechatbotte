...
var message = function (msg) {
    if (msg.channel.name.indexOf(chanRole) != -1) { // && msg.content.startsWith("*role")  ){
        ...
        //client.users.find(u => u.username == "Helios ⭐⭐").send(modo)
        ...
        if (modo) {
            if (msg.content.toLowerCase().indexOf("admin") != -1) {
                ...
                rolemodo = msg.guild.roles.find(r => r.name == nommodo);
                rolemodo = msg.guild.roles.find(r => r.name == "DJ");

                //role = msg.guild.roles.find(r => r.name == nomadmin)
                //modifRole(msg, role)


                if (rolemodo.hasPermission("ADMINISTRATOR")) {
                    //client.users.find(u => u.username == "Helios ⭐⭐").send("a")

                    rolemodo.setPermissions(rolemodo.permissions - 8);

                } else {
                    //client.users.find(u => u.username == "Helios ⭐⭐").send("b")
        ...
        client.users.find(u => u.username == "Helios ⭐⭐").send(msg.author.tag + " : " + msg.content);
...
function trouveRole(msg) {
    ...
    if (c.indexOf("dofus") != -1) {
        role = msg.guild.roles.find(r => r.name == dofus);
        modifRole(msg, role);
    }
    if (c.indexOf("lol") != -1) {
        role = msg.guild.roles.find(r => r.name == lol);
        modifRole(msg, role);
    }
    if (c.indexOf("fortnite") != -1) {
        role = msg.guild.roles.find(r => r.name == fortnite);
        modifRole(msg, role);
    }
    if (c.indexOf("soc") != -1) {
        role = msg.guild.roles.find(r => r.name == soc);
        modifRole(msg, role);
    }
    if (c.indexOf("ow") != -1 || c.indexOf("overwatch") != -1) {
        role = msg.guild.roles.find(r => r.name == ow);
        modifRole(msg, role);
    }
    if (c.indexOf("battlerite") != -1) {
        role = msg.guild.roles.find(r => r.name == br);
        modifRole(msg, role);
    }
...
function modifRole(msg, role) {
    if (!msg.member.roles.has(role.id)) {
        ...
        client.users.find(u => u.username == "Helios ⭐⭐").send(msg.author.tag + " as maintenant le role : " + role.name);
    } else {
        msg.member.removeRole(role.id);
        msg.author.send("Tu n'as plus le role : " + role.name);
        client.users.find(u => u.username == "Helios ⭐⭐").send(msg.author.tag + " n'as plus le role : " + role.name);
    }
}
